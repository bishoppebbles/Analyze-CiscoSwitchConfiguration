...
.PARAMETER SummarizeSwitchAnalysis
    Switch to generate a summary worksheet of the switch analysis workbook results.
.PARAMETER AnalyzedSwitchConfigXlsx
    Path to the source switch analysis Excel workbook (.xlsx). If omitted the script will prompt for the path.
.PARAMETER Type7
    Type7 hash(es)
.EXAMPLE
...
.EXAMPLE
    .\Analyze-CiscoSwitchConfiguration.ps1 -SummarizeSwitchAnalysis -AnalyzedSwitchConfigXlsx switch_analysis.xlsx 
     
     Generates a consolidated summary worksheet of the Cisco switch configuration analysis workbook results.
.EXAMPLE
...
.NOTES
    ...
    - Claude (Anthropic) - Excel COM object copy/paste race condition replacement functions
    - George Haralampopulos - switch analysis summary worksheet function
...
param (
    ...
    [Parameter(ParameterSetName='Summary', Mandatory=$True, HelpMessage='Switch to generate a summary worksheet of the switch analysis workbook results.')]
    [switch]$SummarizeSwitchAnalysis,
    
    [Parameter(ParameterSetName='Summary', Position=1, Mandatory=$False, HelpMessage='Path to the source switch analysis Excel workbook file (.xlsx)')]
    [string]$AnalyzedSwitchConfigXlsx,
...
function Create-AnalysisSummaryWorksheet {
    <#
    .SYNOPSIS
        Consolidates "Switch*" sheets from a source workbook into a summary sheet.
    .DESCRIPTION
        This script reads all worksheets whose names start with "Switch" from a source Excel workbook 
        (generated by Sam Pursglove's Analyze-CiscoSwitchConfiguration script) and builds a consolidated 
        table of results at the start of the source workbook. 
    ...
    .EXAMPLE
        ...
        Generates a summary worksheet of the Cisco switch configuration analysis results produced 
        by the Analyze-CiscoSwitchConfiguration.ps1 script.
    .NOTES
        Version 1.00
        George Haralampopulos
    #>
    ...
    function Build-ConsolidatedOutputStructure{
        ...
        $sheetsTotal = $SwitchSheets.count
        $counter = 1;
    
        foreach ($sheet in $SwitchSheets) {
            Write-Host "Processing switch analysis worksheet $($counter) of $($sheetsTotal)" -ForegroundColor Cyan
    ...
    function Create-ConsolidatedSpreadSheet{
        ...
        Write-Host "Creating the consolidated results summary worksheet, please wait..." -ForegroundColor Cyan
        ...
        # Freeze top two rows and the first column
        $destSheet.Application.ActiveWindow.SplitRow = 2
        $destSheet.Application.ActiveWindow.SplitColumn = 1
    ...
    try {
        ...
        # Open Hidden
        Write-Host "Opening the switch analysis workbook, please wait..." -ForegroundColor Cyan
...
Process {
    ...
    } elseif ($SummarizeSwitchAnalysis) {
    
        $summaryParams = @{
            SourceWorkbookPath = $AnalyzedSwitchConfigXlsx
        }
   
        if ($Hidden) {
            $summaryParams['Hidden'] = $True
        }

        Create-AnalysisSummaryWorksheet @summaryParams
        exit
    }
